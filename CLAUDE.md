...
### 1. 경쟁사 비교 — 섹션별 랭킹 중복 표시 버그 (커밋 `9eca0ad`, NCP 배포 완료)

- **증상**: `/compare` 섹션별 랭킹 탭에서 같은 기사가 2위·3위에 동시에 찍히는 등 순번이 이상하게 보임 (세계일보 IT/과학 섹션에서 "차세대 중형위성 4호" 기사가 rank=2, rank=3 양쪽에 중복 확인).
- **원인**: [collect_section_ranking.py](scripts/collect_section_ranking.py)가 `(media_company_id, section_name, rank, ranking_date)` 단위로 upsert. 기사가 랭킹을 이동해도(예: 3위→2위) 예전 rank 자리를 이번 수집에서 다른 기사가 새로 차지하지 않으면 그대로 남아, 같은 기사가 여러 rank에 중복 적재됨.
- **경로 확인**: 섹션별 랭킹·댓글 랭킹 둘 다 `cluster_articles.py`/`detect_gap.py`(이슈 모니터링·낙종 탐지)에서 전혀 참조되지 않음 — `/compare` 표시 전용. `article` 테이블에 기사를 적재하는 스크립트는 `collect_ranking.py`(경쟁사, 인기랭킹 top20)·`collect_publications.py`(자사, 전체발행) 둘뿐 — 경쟁사 낙종 탐지는 "인기 랭킹 20위 안" 보도만 대상이라는 사각지대도 재확인(48차 이월 이슈와 동일).
- **수정**: `_delete_stale_ranks()` 추가 — upsert 먼저 하고, 이번 수집 결과에 없는 rank만 그 다음에 삭제. (삭제를 먼저 하면 수집 실패 시 데이터가 통째로 비는 위험이 있어 순서를 반대로 함) 세계일보 로컬 실행 + 가짜 스테일 행 주입 테스트로 정상 삭제 확인.
- **배포**: git push → GitHub Actions 이미지 빌드 → infra-mcp `deploy_worker` 완료. 세계일보는 즉시 정리 확인, 다른 매체(media_company_id 2, 14 등)는 다음 정기 크론(매시 7분)에서 자동 정리됨 — 사용자가 대기하기로 함(다음 세션 확인 필요).

### 2. 실시간 트렌드 — InfoTip 툴팁 텍스트 넘침 버그 (커밋 `03e279c`, Vercel 배포 완료)

- **증상**: 증가율·신선도·보도 등 헤더의 (i) 아이콘 호버 시 설명 텍스트가 박스 밖으로 삐져나옴.
- **1차 시도(되돌림, 커밋 `087dc21`)**: 화면 경계 clamp + 아래 공간 부족 시 위로 자동 전환하는 로직을 추가·배포했으나, 이 페이지는 테이블 헤더 위(스탯카드)·아래(데이터행) 모두 콘텐츠가 빽빽해서 겹침 위치만 옮겨졌을 뿐 근본 해결이 아니었음. 사용자 피드백으로 되돌림.
- **진짜 원인**: `<th className="whitespace-nowrap">`(테이블 헤더)의 `white-space: nowrap`이 CSS 상속 속성이라, 툴팁이 `position:fixed`로 화면에 떠도 DOM상 부모-자식 관계는 그대로라 nowrap이 그대로 내려와 줄바꿈이 막혀 224px 박스 테두리를 뚫고 나감.
- **수정**: 위치 로직은 원래 방식(아이콘 바로 아래, 중앙정렬)으로 되돌리고, `whitespace-normal` 클래스 한 줄만 추가해 상속을 끊음. Playwright로 실제 구조를 재현한 정적 HTML을 만들어 전/후 스크린샷 비교로 검증(로그인 미들웨어 때문에 실제 앱 화면은 직접 캡처 못 함).
- **배포**: Vercel 자동배포(GitHub 연동) 완료.

**판단 사항 (53차)**:
1. **삭제는 upsert 이후에** — delete-then-insert는 수집 실패 시 데이터가 아예 비거나, 삭제·삽입 사이 빈 화면이 노출될 위험이 있어 "upsert 먼저 → 이번에 없는 값만 나중에 삭제" 순서로 결정.
2. **InfoTip 위치 보정은 과설계였음** — 화면 경계·주변 콘텐츠 충돌을 전부 피하려는 범용 위치 로직보다, 실제 원인(CSS 상속)만 정확히 짚어 최소 수정하는 쪽이 맞았음. "위치" 문제로 성급히 단정하지 않고 사용자 재확인을 거친 뒤 방향을 바꾼 게 유효했음.
3. **경쟁사 낙종 탐지 사각지대(인기랭킹 20위 밖)는 여전히 미해결** — 48차부터 이월된 방향 결정 보류 사항, 오늘은 재확인만 하고 손대지 않음.

### 미완료 / 다음 세션(54차) 할 일

- ⚠ **섹션별 랭킹 스테일 정리 — 전 매체 확인** (신규) — 다음 크론(매시 7분) 이후 세계일보 외 매체(특히 media_company_id 2, 14)도 정상 정리됐는지 확인.
- ⚠ **2023 백필 11·12월** (52차 이월) — 월 단위로 이어서.
- ⚠ **Supabase Pro 업그레이드 결정** (52차 이월) — `woos2324's` 조직.
- ⚠ **미보도 탐지 수집 범위 결정 (보류)** (48차 이월) — 경쟁사 전 매체 전체 발행 수집으로 확장할지 여부.
...
**이번 세션 (47차) = ① 섹션별 랭킹뉴스 수집 주기를 네이버 섹션 랭킹 업데이트 주기에 맞춰 매시 7분으로 조정, ② 트래픽 주간/월간 PV 누락 4건 네이버 재수집 백필. DB 마이그레이션 없음.**

### 1. 섹션별 랭킹뉴스 매시간 수집 전환

- [crontab](crontab) — `collect_section_ranking` 실행 시각을 **매시 7분 UTC**로 변경. 인기 랭킹(`collect_ranking`)과 동일 시각에 돌려 네이버 랭킹/섹션 랭킹 스냅샷 기준 시각을 맞춤.
- [CLAUDE.md](CLAUDE.md) 자동화 파이프라인 표 — `cron-section-ranking`을 “매시 7분 (UTC) — 47차 변경”으로 반영.
- 수집 대상/저장 구조는 기존 그대로 유지: 섹션별 랭킹 → `section_ranking_snapshot`.

### 2. 트래픽 주간/월간 PV 누락 4건 백필

- **증상**: `/traffic` 총조회수 더보기에서 주간 `5/18~5/24`·`5/25~5/31`·`6/8~6/14`, 월간 `5월`이 0/누락(`—`)으로 표시.
- **원인**: 주간(월요일)·월간(1일) PV는 KST 05:00 단 1회만 수집되고 재시도/fallback이 없음(일간만 07:00 fallback 보유). 해당 회차가 NCP 이전기(33차 전후)·6월 세션 302 장애(44차) 시간대에 걸려, 호출이 0을 받거나(행은 생기되 pv=0) 예외로 죽어(행 자체 부재) 영구 잔존. 일간은 이후 정상 복구됐으나 주간/월간은 자동 교정 안 됨.
- **조치**: 워커 컨테이너에서 `collect_naver_pv --weekly --week-date {20260518,20260525,20260608}` + `--monthly --month-date 20260501` 재수집 → 네이버 공식 집계로 백필 완료. 검증: 주간값이 일간 7일 합계와 정확히 일치(5/18=9,372,344 등), 5월 월간=40,681,534. `revalidate("traffic")` 자동 호출됨.
- **실행 방식 메모(중요)**: 워커에서 스크립트를 수동 실행하려면 **컨테이너 안에서** 돌려야 함 — `docker exec worker-worker-1 sh -c "cd /app && python -m scripts.X"`. 앱 코드는 호스트(`/home/segyecom/worker`)가 아니라 **이미지 `/app`에** 있음. 호스트엔 `python` 없이 `python3`만 있고 코드가 없어, 호스트에서 `python3 -m scripts.X` 하면 `No module named 'scripts'` 발생(이미지 손상 아님). infra-mcp는 docker compose만 허용 → paramiko 직접 SSH(`D:\mcp\infra-mcp\config.yaml`).

**판단 사항 (47차)**:
1. **네이버 섹션 랭킹은 매시간 갱신 기준으로 본다** — 일/몇 회 수집보다 매시 스냅샷이 랭킹 변화 추적에 적합.
2. **인기랭킹과 동일한 `:07` 실행** — 두 랭킹 계열의 비교 기준 시각을 맞추고, 기존 ranking cron 운영 패턴을 재사용한다.
3. **주간 백필도 SQL 합산 대신 네이버 재수집으로 통일** — 월간은 daily_cv가 5/17부터만 있어 합산 불가 → 어차피 네이버 재수집 필요. 주간도 동일 경로로 통일해 일관성 확보(6/1주로 네이버 WEEK = 일간 합계 정확성 검증됨).

### 미완료 / 다음 세션(48차) 할 일

- ⚠ **주간/월간 PV 수집 fallback 추가 (재발 방지)** — 주간(월요일)·월간(1일) 수집은 KST 05:00 단 1회·재시도 없음 → 장애 시간대에 걸리면 0/누락 영구 잔존(47차에 4건 백필했으나 구조는 그대로). **조치 후보**: ① 수집 시각을 새벽보다 늦춰 네이버 집계 확정 이후로, 또는 ② 0/누락 시 다음날 자동 재시도(일간 07:00 fallback과 유사). `collect_period`는 예외 시 WARN만 찍고 적재 생략하므로 실패 회차는 행 자체가 안 생김(0이 아니라 누락).
...
**미완료 / 다음 세션 이어받을 것**:
- ⚠ **opinion "세계일보 트렌드"(`/trend`) UI 개선** (2건):
  1. 자사(세계일보) 사설 목록의 **"더보기" 버튼 → 페이징 처리**로 변경
  2. 다른 메뉴(오늘의 사설 등)에 있는 **날짜 선택(DateNav, `basePath="/trend"`) 추가**
  - 착수 전 현재 `opinion/src/app/trend/` 구조(더보기 동작·날짜 처리 여부) 파악 필요. DateNav 컴포넌트는 기존 재사용([opinion/src/components/DateNav.tsx](opinion/src/components/DateNav.tsx), `basePath` prop 보유)
